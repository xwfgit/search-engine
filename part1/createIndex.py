...
'''    
    def compute_tf(self, id_page_dict):
        term_pagetf_dict = dict()
        pageID_norm = dict()
        for id, text in id_page_dict:
            sum = 0;
            term_list = text.split(' ')
            for term in term_list:
                page_tf_dict = dict()
                page_pos_dict = self.inverted_index[term]
                pos_list = page_pos_dict[id]
                print pos_list
                tf = 1 + math.log10(len(pos_list))
                sum += tf ** 2
                page_tf_dict[id] = tf
                term_pagetf_dict[term] = page_tf_dict
            pageID_norm[id] = math.sqrt(sum)
'''    
